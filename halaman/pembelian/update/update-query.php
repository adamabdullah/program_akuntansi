...
if(empty($errorMSG)){
    ...
    $sql = "UPDATE `transaksi_produk` SET `pelanggan`='".$kontak."', `tgl_transaksi`='".$tgl_transaksi."', `kode`='".$nomor_transaksi."', `tgl_tempo`='".$tgl_tempo."',  `memo`='".$memo."' , `pesan`='".$pesan."', `sisa_tagihan`='".$tagihan_baru."', `total`='".$total_semuanya."' WHERE kode='".$kode_rahasia."'";
    ...
    //untuk akun utama
        $sql_kirim = "INSERT INTO `transaksi`(`kode_transaksi`, `kode_akun`, `kolom`, `kredit`, `debit`, `no` , `tgl_transaksi`) VALUES ('".$nomor_transaksi."', '2-20100 | Hutang Usaha', 'pembelian', '".$total_semuanya."' ,'0','".$noUrut."', '".$tgl_transaksi."')";
...
if($sum > 0)
{
    ...
    foreach ($nama_pajak_total1 as $key=>$isi) 
    {
        if($uang_pajak_total1[$key] > 0)
        {
            ...
            $sql_kirim1 = "INSERT INTO `transaksi`(`kode_transaksi`, `kode_akun`, `kolom`, `kredit`, `debit`, `no` , `nama_pajak_ori`, `tgl_transaksi`) VALUES ('".$nomor_transaksi."', '".$data_pajak['akun_pajak_pembelian']."', 'pembelian_pajak','0', '".$uang_pajak_total1[$key]."','".$noUrut."', '".$nama_pajak_total1[$key]."', '".$tgl_transaksi."')"; 
    ...
    foreach ($nama_produk as $key => $value) 
    {
        if ($nama_produk[$key] != '') 
        {
            ...
            $sql_kirim1 = "INSERT INTO `transaksi`(`kode_transaksi`, `kode_akun`, `kolom`, `kredit`, `debit`, `no`, `nama_pajak`, `harga_pajak`, `jumlah_uang`, `qty_produk`, `nama_produk`, `nama_pajak_ori`, `tgl_transaksi`) VALUES ('".$nomor_transaksi."', '".$data_produk['akun_jual']."', 'pembelian','0', '".$jumlah_uang_produk[$key]."','".$noUrut."', '".$data_pajak['akun_pajak_pembelian']."', '".$uang_pajak[$key]."', '".$harga_satuan[$key]."', '".$quantity[$key]."', '".$nama_produk[$key]."',  '".$nama_pajak[$key]."', '".$tgl_transaksi."')";
            mysqli_query($connect, $sql_kirim1);   
        }
        else
        {

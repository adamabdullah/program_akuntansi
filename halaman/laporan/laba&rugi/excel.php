...
if(isset($_POST['first'])&&isset($_POST['last'])){
    ...
    $sel_akun_4 = "SELECT kode_akun, kredit-debit AS total from transaksi WHERE kode_akun like '4%' AND tgl_transaksi between '$f' AND'$l'";
    $res_akun_4 = mysqli_query($connect, $sel_akun_4);
    
    $sel_akun_5 = "SELECT kode_akun, kredit-debit AS total from transaksi WHERE kode_akun like '5%' AND tgl_transaksi between '$f' AND'$l'";
    $res_akun_5 = mysqli_query($connect, $sel_akun_5);
    
    $sel_akun_6 = "SELECT kode_akun, kredit-debit AS total from transaksi WHERE kode_akun like '6%' AND tgl_transaksi between '$f' AND'$l'";
    $res_akun_6 = mysqli_query($connect, $sel_akun_6);
    
    $sel_akun_7 = "SELECT kode_akun, kredit-debit AS total from transaksi WHERE kode_akun like '7%' AND tgl_transaksi between '$f' AND'$l'";
    $res_akun_7 = mysqli_query($connect, $sel_akun_7);
    
    $sel_akun_8 = "SELECT kode_akun, kredit-debit AS total from transaksi WHERE kode_akun like '8%' AND tgl_transaksi between '$f' AND'$l'";
...
header("Content-Disposition: attachment; filename=Laporan Laba & Rugi ($f to $l).xls");
...
<table class="table table-hover" cellpadding="5">
    ...
    <tr>
        <th>Tanggal</th>
        <th class="pull-right"><?php echo $f." sampai ".$l; ?></th>
    </tr>
    </thead>

						...
						<div class="row">
							...
							<!-- /.form-group -->
								...
								<!-- <div class="col-md-3">
									...
									<input type="text" id="tag" class="form-control" value="<?php echo $data_1['tag_2']; ?>" placeholder="Auto" aria-describedby="basic-addon1">
								</div> -->
						</div>
						<!-- /.row -->
	...
	$(document).ready(function() 
{
	...
	$(document).on('click', '#nambah_pajak_baru_terima_uang',function() //------------------------------untuk mengecek (akun)
	{
		$("#modal_buat_pajak").modal('toggle');
	});

	$(document).on('click', '#akun_terima_dari_tabel_baru, #kontak_penerima_modal_akun',function() //------------------------------untuk mengecek (akun)
	{
		$("#modal_nambah_akun").modal('toggle');
	});
	...
	// var deskripsi = deskripsi1.split(',');

	// var terima_dari=[];
	// $.each(terima_dari2,function(index,obj)
	// {
	// 	terima_dari[index] = terima_dari2[index].trim();
	// });
	// alert(terima_dari);
	...
	if(panjang >= 1)
	{
		...
		$.each(terima_dari,function(index,obj)
		{
			...
			if(index == 0)
			{
				...
				$("#tabel_terima_uang tbody tr:eq("+index+") td .akun_append").val(obj.trim());
				...
				// $("#tabel_terima_uang tbody tr:eq("+index+") td .deskripsi").val(deskripsi[index]);
			...
			{	 
				...
				success:function(data) 
				{
					...
					$("#tabel_terima_uang tbody tr:eq(1) td .akun_append").val(obj.trim());
					var myString = nama_pajak_ori[index].substr(nama_pajak_ori[index].indexOf("|") + 1);
					var angka_pajak2 = myString.split("%").join("");
					$(".uang").mask('#,##0.00', {reverse: true});	
					// $("#tabel_terima_uang tbody tr:eq(1) td .deskripsi").val(deskripsi[index]);
	...
	$(document).on('click', '.kurangi', function(e)
	{
		...
		var rowIndex = $('#tabel_terima_uang tr').index($(this).closest('tr'));
 		...
 		$.ajax({
				url:"/faktur_v2/halaman/kas/update_terima/delete-query/index.php",
				method:"POST",
 	...
 	$("#beli_biaya").click(function()
 	{
		...
		// var deskripsi = [];
 		...
 		$.ajax({
			url:"/faktur_v2/halaman/kas/update_terima/update-query/index.php",
			method:"POST",
			dataType:'json',
			data:{total_uang_tnpa_koma:total_uang_tnpa_koma, tgl_transaksi:tgl_transaksi, akun_bayar:akun_bayar, penerima:penerima, no_biaya:no_biaya, memo:memo, nama_pajak_total:nama_pajak_total, uang_pajak_total:uang_pajak_total, akun_val:akun_val, uang_val:uang_val, kode_rahasia:kode_rahasia},
			success:function(data) 
			{
				// alert(data);
				console.log(data);

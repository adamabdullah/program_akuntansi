...
$data_pajak['akun_pajak_penjualan'];

//untuk mengambil nilai pajak spesifik
$pajak_masukkan2 = "SELECT * from transaksi where nama_pajak_ori='".$nama_pajak_spesifik[0]."' and kode_akun='".$data_pajak['akun_pajak_penjualan']."' and kode_transaksi='".$kode_transaksi."'";
$hasil_query2 = mysqli_query($connect,$pajak_masukkan2);
$data_pajak2= mysqli_fetch_array($hasil_query2);
$uang_pajak_ambil_db = $data_pajak2['kredit'];
$stlh_dikurangi = $uang_pajak_ambil_db-$uang_pajak_spesifik[0];

//untuk mengambil nilai kredit semua total
$total_keseluruhan = "SELECT * from transaksi where kode_akun='1-10100 | Piutang Usaha' and kode_transaksi='".$kode_transaksi."'";
$hasil_query2 = mysqli_query($connect,$total_keseluruhan);
$data_uang_utama= mysqli_fetch_array($hasil_query2);
$total_seluruh_akun = $data_uang_utama['debit'];
...
$update_kirim_uang = "UPDATE `transaksi` set `kredit`='".$stlh_dikurangi."' where nama_pajak_ori='".$nama_pajak_spesifik[0]."' and kode_akun='".$data_pajak['akun_pajak_penjualan']."' and kode_transaksi='".$kode_transaksi."' ";
mysqli_query($connect, $update_kirim_uang);

//untuk mengurangi akun utama kredit, contoh 1-10002 | Rekening Bank
$update_kredit_utama = "UPDATE `transaksi` set `debit`='".$hasil_debit."' where  kode_akun='1-10100 | Piutang Usaha' and kode_transaksi='".$kode_transaksi."' ";
mysqli_query($connect, $update_kredit_utama);
//--------------------------------------------------------

//delete ketika pajak sudah 0
$sql_menghapus_null= "DELETE FROM `transaksi` where kode_akun='".$data_pajak['akun_pajak_penjualan']."' and `kode_transaksi`= '".$kode_transaksi."' and `kredit`=0";
mysqli_query($connect, $sql_menghapus_null);

//delete ketika debit utama sudah 0
$sql_kredit_null= "DELETE FROM `transaksi` where `nama_produk`='".$nama_produk."' and `kode_transaksi`= '".$kode_transaksi."' and `debit`=0 and `kode_akun` = '1-10100 | Piutang Usaha'";

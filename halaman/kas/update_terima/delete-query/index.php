...
//untuk mengambil nilai debit semua total
...
$update_kirim_uang = "UPDATE `transaksi` set `kredit`='".$stlh_dikurangi."' where nama_pajak_ori='".$nama_pajak_spesifik[0]."' and kode_akun='".$data_pajak['akun_pajak_pembelian']."' and kode_transaksi='".$kode_transaksi."' ";
mysqli_query($connect, $update_kirim_uang);
//------------------------------------------------------

//untuk mengurangi akun utama debit, contoh 1-10002 | Rekening Bank
$update_kredit_utama = "UPDATE `transaksi` set `debit`='".$hasil_kredit."' where kode_akun='".$akun_asal."' and kode_transaksi='".$kode_transaksi."' ";
...
//delete untuk akunnya
$sql_hapus_pajak = "DELETE from `transaksi` where `kode_transaksi`='".$kode_transaksi."' and `nama_pajak_ori`='".$nama_pajak_spesifik[0]."' and `kredit`='".$uang_per_akun."' ";
...
$sql_menghapus_null= "DELETE FROM `transaksi` where kode_akun='".$data_pajak['akun_pajak_pembelian']."' and `kode_transaksi`= '".$kode_transaksi."' and `kredit`=0";
mysqli_query($connect, $sql_menghapus_null);

//delete ketika kredit utama sudah 0
$sql_kredit_null= "DELETE FROM `transaksi` where `kode_akun`='".$akun_asal."' and `kode_transaksi`= '".$kode_transaksi."' and `debit`=0";
